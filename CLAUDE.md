...
- Sample data is generated by scripts/generate_sample_data.py (seeded,
  deterministic): a LARGE stress term — 60 students / 10 teachers /
  2026-07-21..08-31 minus Sundays × 6 periods (216 slots) / one
  12-seat hall (teacher limit 6) / ~3 subjects per student, EXACTLY 5
  sessions each (~900 lessons);
  all teachers teach jpn/eng/soc, only five teach math/sci. Slot ids
  look like
  `0801-1` (= MMDD-period). v1 copes thanks to unique-pair MRV with
  early-exit candidate
  counting (and a raised recursion limit); `optimize_teacher_days`
  SKIPS instances over 400 movable lessons (quality at scale comes from
  CP-SAT). Solver-behavior tests use their own small fixtures
  (`medium_dataset` in test_solver_v2), NOT the shipped sample. `POST /api/timeslots/bulk`
