...
def test_cpsat_honors_subject_assignment():
    """Assignments are enforced in the model itself: no variable is
    even created for other teachers on that (student, subject)."""
    d = make_data()
...
def test_cpsat_assignment_restricts_only_that_subject():
    """s1's math is t2-only, but s1's eng may go to anyone."""
    d = make_data()
    d.subject_teachers = {("s1", "math"): {"t2"}}
    d.student_needs = {("s1", "math"): 1, ("s1", "eng"): 1}
    r = solve_v2(d)
